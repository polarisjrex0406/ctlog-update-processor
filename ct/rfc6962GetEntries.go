...
func (ge *getEntries) callRFC6962GetEntries() {
	...
	for {
		...
		var err error
		outputDir := "/root/_zzz_/scrape-ct-log/target/release/tmp/"
		filenamePrefix := "log_entries"
		if err = utils.Scrape(outputDir, logURL, filenamePrefix, ge.ctLogID, start, end-start+1); err != nil {
			logger.Logger.Error("scrape-ct-log execution failed", zap.Error(err))
		} else {
			// Load JSON file (adjust path as needed)
			jsonFile := fmt.Sprintf("%s%s_%d(%d-%d).json", outputDir, filenamePrefix, ge.ctLogID, start, end)
			if body, err = os.ReadFile(jsonFile); err != nil {
				logger.Logger.Error("os.ReadFile failed", zap.Error(err), zap.String("logURL", logURL), zap.Int64("start", start), zap.Int64("end", end))
			} else {
...
func (ge *getEntries) processNewRFC6962Entries(body []byte, start int64, processedEntries *[]msg.NewLogEntry) int64 {
	...
	for _, entry := range ger.Entries {
		...
		for i := len(entry.Chain) - 1; i >= 0; i-- {
			// Step 1: Trim whitespace and special characters
			trimmed := strings.TrimSpace(entry.Chain[i])
			...
			if nle.DerCert, err = base64.StdEncoding.DecodeString(trimmed); err != nil {
			...
			if nle.Cert, err = x509.ParseCertificate(nle.DerCert); err != nil {
				logger.Logger.Warn("Could not parse chain certificate", zap.Error(err), zap.String("logURL", ctlog[ge.ctLogID].Url), zap.Int64("index", entry.EntryNumber), zap.Time("timestamp", ctgo.TimestampToTime(entry.Timestamp).UTC()))
				nle.Cert = nil
		...
		// Step 1: Trim whitespace and special characters
		trimmed := strings.TrimSpace(entry.Certificate)
		// Step 2: Add missing padding if needed
		if missing := len(trimmed) % 4; missing != 0 {
			trimmed += strings.Repeat("=", 4-missing)
		}
		// Decode base64 certificate (DER format)
		derCert, err := base64.StdEncoding.DecodeString(trimmed)
		if err != nil {
			logger.Logger.Error("Could not decode entry", zap.Error(err), zap.String("logURL", ctlog[ge.ctLogID].Url), zap.Int64("index", index))
			return index
		}

		// Process the certificate or precertificate entry.
		nle.CtLogID = ge.ctLogID
		nle.EntryID = entry.EntryNumber
		nle.EntryTimestamp = ctgo.TimestampToTime(entry.Timestamp).UTC()
		nle.DerCert = derCert
		nle.Sha256Cert = sha256.Sum256(nle.DerCert)
		nle.IssuerVerified = false
		if nle.Cert, err = x509.ParseCertificate(nle.DerCert); err != nil {
			logger.Logger.Warn("Could not parse certificate", zap.Error(err), zap.String("logURL", ctlog[ge.ctLogID].Url), zap.Int64("index", entry.EntryNumber), zap.Time("timestamp", nle.EntryTimestamp))
			nle.Cert = nil
		} else if issuerCert != nil {
			if nle.Cert.CheckSignatureFrom(issuerCert) == nil {
				// Signature is valid, so pass the parent certificate's SHA-256 hash.
